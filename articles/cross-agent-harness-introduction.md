...
AI エージェントを 1 台で使うだけなら、会話の流れで「次に何をするか」を維持できます。ところが 2 台以上を同じリポジトリで動かすと、作業範囲、レビュー結果、検証状況、公開判断が会話ごとに分散します。人間のチーム開発で issue、レビュー、CI、リリース条件を分けるのと同じように、AI エージェント同士の作業にも戻る場所が必要になります。
...
ただし、導入して終わりではありません。次に必要になるのは、対象プロジェクトごとの判断を profile と handoff に寄せていくことです。
...
```markdown:project-collaboration-profile.template.md
## 担当境界
...
```markdown:CLAUDE_CODE_HANDOFF.template.md
### 完成条件（スプリントコントラクト）
...
```markdown:.agents/skills/implement-task/SKILL.md
## 開始時
...
`cross-agent-harness` を入れたあとに詰まるのは、スクリプトの実行よりも運用の調整です。

まず見るのは `project-collaboration-profile.md` です。担当境界が曖昧なままだと、Codex と Claude Code のどちらに渡すべきかを毎回その場で決めることになります。最初は完璧な表を作らなくてもよく、次の3点だけ埋めると使い始めやすくなります。

- 通常の verify コマンド
- 触る前に確認すべき危険領域
- merge / publish / release を誰の判断で進めるか

次に見るのは `CLAUDE_CODE_HANDOFF.md` です。実装が進んだのにレビュー担当が状況を読めない場合、依頼範囲、実行した verify、未確認の項目が足りていないことが多いです。handoff には「何をしたか」だけでなく、「何をまだ確認していないか」も残すようにしています。

既存の profile や handoff があるプロジェクトでは、すぐに `-Force` で上書きしません。まず既存ファイルを読み、共通ルール、プロジェクト固有ルール、直近ログに分けてから移します。ハーネスは既存運用を消すためのものではなく、散らばった運用を同じ場所へ寄せるためのものです。
...
- [Claude Code運用を数ヶ月で見直してrulesとskillsに分けた話](https://zenn.dev/harness/articles/claude-code-workflow-evolution)
- [AI 2 台クロスレビューで技術記事の盲点を拾う](https://zenn.dev/harness/articles/ai-cross-review-handoff-workflow)
- [AIとの設計判断をMy-Skill-Graphに残して再利用する](https://zenn.dev/harness/articles/codex-claude-skill-graph-worklog)
