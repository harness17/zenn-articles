...
ここでいう OAuth は、ユーザーの Google アカウントで許可を取り、登録チャンネルなどの本人向けデータへアクセスする仕組みです。`credentials.json` は、その認証に使うアプリ側の設定ファイルです。どちらも開発者には一般的ですが、配布アプリの初回体験としてはかなり重い前提になります。
...
RSS と API で取れる情報は、かなり違います。

| 取得方法 | 取れる情報 | 苦手なこと |
| --- | --- | --- |
| RSS | チャンネルの新着動画、タイトル、URL、公開時刻 | 登録チャンネルの自動同期、配信予定時刻、ライブ状態の安定取得 |
| YouTube Data API | 登録チャンネル、配信予定時刻、ライブ状態、動画詳細 | OAuth 設定、クォータ管理、認証エラー対応 |
...
ここまでが、アプリ内部の設計判断です。ただ、個人開発アプリを他の人に触ってもらう段階では、取得方式だけでなく、認証ファイル、署名、クォータといった配布時の課題も体験に直結します。
