...
実際にアプリを起動するとクォータ枯渇まで一発で到達し、`403 quotaExceeded` が返り始めました。クォータのリセットは太平洋時間 0:00 です。日本時間では夏時間中は 17:00、標準時間中は 16:00 頃なので、それまでは何もできません。
...
- 「消えないバナー」を画面上部に常時表示
- リセット時刻を JST で計算して表示（PT 0:00 → 夏時間中は JST 17:00、標準時間中は JST 16:00 頃）
